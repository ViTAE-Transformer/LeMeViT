...
def blur_2d(
    input: torch.Tensor, stride: _size_2_t = 1, filter: Optional[torch.Tensor] = None
) -> torch.Tensor:
    ...
    return F.conv2d(
        input, filter, stride=stride, padding=padding, groups=n_in_channels, bias=None
    )
...
class BlurConv2d(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        if self.blur_first:
            ...
            return blur_2d(
                activations, filter=self.blur_filter, stride=self.blur_stride
            )
...
def restore_blurpool(mod: nn.Module, device):
    with torch.no_grad():
        for name, child in mod.named_children():
            if isinstance(child, BlurPoolConv2d):
                op = BlurPoolConv2d(child.conv)
                if device is not None:
                    op.to(device)
                setattr(mod, name, op)
            else:
                restore_blurpool(child, device)
